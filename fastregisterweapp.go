...
func (s *Server) FastRegisterWeapp(req *FastRegisterWeappReq)(resp *core.Error,err error){
	var(
		p = make(url.Values)
		token string
	)
	...
	err=core.PostJson(WeAPPRegisterUrl+p.Encode(),req,resp)
	return
}
...
//查询创建任务状态
//https://developers.weixin.qq.com/doc/oplatform/Third-party_Platforms/Mini_Programs/Fast_Registration_Interface_document.html
func (s *Server) SearchWeapp(req *SearchWeappReq) (resp *core.Error,err error){
	var(
		p = make(url.Values)
		token string
	)
